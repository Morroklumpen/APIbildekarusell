...
const { setTokenBan, isTokenBanned } = require("../handlers/redishandler");
...
const authenticate = async (req, res, next) => {
  ...
  if (typeof accesstoken !== "undefined" && typeof accesstoken === "string") {
    try {
      const { _id, cryptotoken } = jwt.verify(
        accesstoken,
...
const invalidateTokens = async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      const { _id, cryptotoken } = await jwt.verify(
        refreshToken,
        process.env.JWTSECRET
      );
      ...
      if (!result) {
        ...
        console.warn(
          "Tried deleting a refreshtoken, but token vas not in the database!"
        );
      }
    } else {
    ...
    if (typeof accessToken !== "undefined" && typeof accessToken === "string") {
      const { cryptotoken, exp } = await jwt.verify(
        accessToken,
        process.env.JWTSECRET
      );
      ...
      if (feedback.payload) {
        feedback = createFeedback(
          404,
          feedback.payload.push("invalid refresh token")
        );
      } else {
